...
class UserController extends Controller
{
	...
	public function select_hr(Request $request)
	{
		...
		foreach ($hrs->get() as $hr) {
			if ((bool)$request->mail_tag && (bool)$hr->getMail($request->mail_tag, "noibo")) {
				$text = '[' . $hr->key . '] ' . $hr->hoten . " (" . $hr->getMail($request->mail_tag, "noibo") . ")";
			} else {
	...
	public function login_stnv_action(Request $request)
	{
		...
		$check = DB::table('jobs')->where('payload', 'like', '%iCPC1HN_Group_Sync_Job%')->count();
		if ($check <= 0) {
